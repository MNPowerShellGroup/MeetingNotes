...
If (test-path -Path "D:\scripts\client_reinstall_sccm_agent.ps1")
{
    $file = Get-ItemProperty -Path "D:\scripts\client_reinstall_sccm_agent.ps1" | HashCheck -erroraction silentlyContinue

    $output = $file.path + "," + $file.'SHA1 hash'

    Write-Output $output
}
If (test-path -Path "D:\scripts\patch_service_regkey_deploy_GUI.ps1")
{
    $file = Get-ItemProperty -Path "D:\scripts\patch_service_regkey_deploy_GUI.ps1" | HashCheck -erroraction silentlyContinue
